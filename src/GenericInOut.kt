...
open class Base {var a: Int = 0}
open class SubBase: Base() {var b: Int = 1}
open class SubSubBase: SubBase() {var c: Int = 2}
open class SubSubSubBase: SubSubBase() {var d: Int = 3}
open class SubSubSubSubBase: SubSubSubBase() {var e: Int = 4}
...
//  val i4: Invariant2<Base>    = Invariant2<Base>()    // Error  Base is not in bounds
    val i5: Invariant2<SubBase> = Invariant2<SubBase>() // OK     SubBase -> SubBase
//  val i6: Invariant2<Base>    = Invariant2<SubBase>() // Error  Base is not in bounds
//  val i7: Invariant2<SubBase> = Invariant2<Base>()    // Error  Base is not in bounds
    ...
    //                    Tt=to                          Tf=from
    val n0: Contravariant<Base>          = Contravariant<Base>()       // OK    Equal classes always work
...
//  val n3: Contravariant<Base>          = Contravariant<SubBase>()    // Error bc Tt is not subclass of Tf
    ...
    val n5: Contravariant<SubSubBase>    = Contravariant<SubBase>()    // OK    as Tt is subclass of Tf
    val n7: Contravariant<SubBase>       = Contravariant<SubBase>()    // OK    Equal classes always work
    val n6: Contravariant<SubSubSubBase> = Contravariant<SubBase>()    // OK    as Tt is subclass of Tf
    val n8: Contravariant<SubSubSubBase> = Contravariant<SubSubBase>() // OK    as Tt is subclass of Tf


    // *****************
    // * <IN T: SubBase> == Contravariant, restricted bounds to specific subclasses/subtypes
    // <in T: allowTypeOrSubtypeOfT>
    // 'in' must be a Subtype of generic param, and restricted to class and subclasses of SubBase, like java <? extends T>
    ...
    val p4: Contravariant2<SubSubBase>    = Contravariant2<SubBase>()     // OK     as Tt is subclass of Tf
...
//  val o9: Contravariant2<SubBase>       = Contravariant2<SubSubBase>()  // Error  bc Tt is not a subclass of Tf
    ...
    // R must be same type or Supertype of generic param, like java <? super R>
...
//  val v2: Covariant<SubBase>    = Covariant<Base>()       // Error  bc Rt is not superclass of Rf
    val v6: Covariant<Base>       = Covariant<SubBase>()    // OK     as Rt is superclass of Rf
...
//    class Covary<in T:SubSubBase, R>(var x: R) {
//        fun f(i: Int): R {return x as R}
//
//        fun f(e: T) {x = e as R}
//    }

    class Covary<in T:SubSubBase, R>(var y: R) {
        fun get(i: Int): R { println("get() ${this.toString()}"); return y}
        fun set(e:T) {y=e as R}
        ...
        override fun toString(): String {
            return (y as T).run {
                "a=${a}, b=${b}, c=${c}, " + super.toString()}
        }
    }
    val aa = Covary<SubSubBase, SubSubBase>(SubSubBase())
//    aa.f(SubBase())
    var x = aa.get(0)
    aa.set(SubSubBase()) // Set a new subBase
